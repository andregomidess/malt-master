...
export class Migration20251127225319_AddsUserInWaterProfile extends Migration {
  override up(): void {
    this.addSql(
    ...
    this.addSql(`alter table "water_profile" add column "user_id" uuid null;`);
    this.addSql(
      `alter table "water_profile" add constraint "water_profile_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade on delete set null;`,
    );
  }
  ...
  override down(): void {
    ...
    this.addSql(
      `alter table "water_profile" drop constraint "water_profile_user_id_foreign";`,
    );
    ...
    this.addSql(
      `alter table "tasting_note" alter column "tasting_date" set default '2025-11-27';`,
    );

    this.addSql(`alter table "water_profile" drop column "user_id";`);
  }
}
